...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build \
    -ldflags="-w -s -X github.com/ASRagab/agentpipe/internal/version.Version=${VERSION:-dev} \
              -X github.com/ASRagab/agentpipe/internal/version.CommitHash=${COMMIT:-unknown} \
              -X github.com/ASRagab/agentpipe/internal/version.BuildDate=$(date -u +%Y-%m-%dT%H:%M:%SZ)" \
    -o agentpipe .
